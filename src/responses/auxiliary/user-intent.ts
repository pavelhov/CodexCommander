...
const NON_EXECUTION_FRAMING = /(?:^|\n)\s{0,3}(?:for\s+example|example|sample|review\s+(?:this|these|the\s+following)|analy[sz]e\s+(?:this|these|the\s+following))\s*:|\bnot\s+(?:an?\s+)?(?:request|instruction|command)\b/im;
...
const RELEVANT_CANCELLATION = /\bnever\s+mind\b|\bcall\s+(?:it|that|this)\s+off\b|\b(?:cancel|withdraw|revoke|retract|rescind|stop|abort|forget|disregard|ignore|scratch|decline|refuse)\s+(?:it|that|this|(?:the|my)\s+(?:request|instruction|generation|video(?:\s+generation)?))\b/i;
