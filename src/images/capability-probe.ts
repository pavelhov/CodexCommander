...
export class CapabilityProbeService {
  ...
  async #runVideo(
    ...
    try {
      ...
      const job = this.#store.findVideoJobForProbe(current.id, confirmationRevision);
  ...
  #settleAcceptedProbeVideo(job: PublicVideoJob): boolean {
    ...
    if (
      ...
      || probe.steps.video.confirmationRevision !== storedJob.confirmationRevision
    ) return true;
  ...
  startBackgroundRecovery(): void {
    ...
    for (const job of this.#store.listVideoJobs()) {
      ...
      if (
        probe?.steps.video.state !== "accepted"
        || probe.steps.video.videoJobId !== job.id
        || probe.steps.video.confirmationRevision !== job.confirmationRevision
      ) continue;
  ...
  async recoverOnStartup(signal?: AbortSignal): Promise<CapabilityProbeStatus[]> {
    ...
    for (const job of this.#store.listVideoJobs()) {
      if (
        job.probeOperationId
        && ["completed", "artifact_pruned", "failed", "expired", "cancelled"].includes(job.state)
      ) candidateIds.add(job.id);
    }
    const operationIds = new Set<string>();
    for (const id of candidateIds) {
      const job = this.#store.getVideoJob(id);
      if (!job?.probeOperationId) continue;
      const probe = this.#store.getCapabilityProbe(job.probeOperationId);
      if (
        !probe
      ...
      } else if (["failed", "expired", "cancelled", "artifact_pruned"].includes(job.state)) {
        this.#store.settleCapabilityProbeStep({
          id: probe.id,
          step: "video",
          expectedStepRevision: probe.steps.video.revision,
          state: "failed",
          dispatchCertainty: "definite_rejection",
          safeError: job.state === "artifact_pruned" ? "download_rejected" : job.safeError ?? "upstream_failed",
          videoJobId: job.id,
        });
      }
