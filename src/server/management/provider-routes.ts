...
function providerReplacementPreservingCredentials(
  replacement: CodexCommanderProviderConfig,
  existing: CodexCommanderProviderConfig | undefined,
): CodexCommanderProviderConfig {
  const next = structuredClone(replacement);
  if (!Object.hasOwn(replacement, "apiKey") && existing?.apiKey !== undefined) {
    next.apiKey = existing.apiKey;
  }
  if (!Object.hasOwn(replacement, "apiKeyPool") && existing?.apiKeyPool !== undefined) {
    next.apiKeyPool = structuredClone(existing.apiKeyPool);
  }
  return next;
}
...
export async function handleProviderRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/providers" && req.method === "POST") {
    ...
    const applyReplacement = (current: CodexCommanderConfig): CodexCommanderConfig => {
      const next = providerReplacementPreservingCredentials(prov, current.providers[name]);
      current.providers[name] = stripRegistryOnlyStaticHeaders(name, next);
      if (body.setDefault === true) current.defaultProvider = name;
      return current;
    };
    let outcome = mutatePersistedConfig(current => ({
      changed: true,
      value: applyReplacement(current),
    }));
    if (outcome.status === "unavailable" && outcome.reason === "missing") {
      withConfigMutationLockSync(() => {
        outcome = mutatePersistedConfig(current => ({
          changed: true,
          value: applyReplacement(current),
        }));
        if (outcome.status !== "unavailable" || outcome.reason !== "missing") return;
        const initial = applyReplacement(structuredClone(config));
        saveConfigPreservingClaudeCode(initial);
        outcome = { status: "committed", value: initial };
      });
    }
    if (outcome.status === "unavailable") {
      return jsonResponse({ error: `provider config ${outcome.reason}` }, 409);
    }
    config.providers[name] = structuredClone(outcome.value.providers[name]!);
