...
/** True when the rewritten request still exposes at least one tool carrier. */
function hasCallableResponsesTools(tools: unknown, input: unknown): boolean {
...
export function rewriteHostedImageGenerationForBridge(
  ...
  let toolChoice = rewriteHostedImageToolChoice(body.tool_choice, mode);
  // `required` is invalid once the forced-final pass removed the only callable tool.
  // Keep it intact when an unrelated top-level or additional_tools declaration remains.
  if (mode === "omit" && toolChoice === "required" && !hasCallableResponsesTools(tools, input)) {
    toolChoice = "auto";
  }
