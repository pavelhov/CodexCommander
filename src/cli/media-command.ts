...
export async function handleMediaCommand(argv: string[], deps: MediaCommandDeps = {}): Promise<number> {
  ...
  try {
    ...
    // `media jobs --json` is a machine-readable jobs document, not a status
    // report followed by a jobs document. Keep the useful status preamble for
    // human jobs output, but never prepend it to JSON stdout.
    if (parsed.command !== "jobs" || !parsed.json) {
