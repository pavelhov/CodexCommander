...
<p align="center">
  <img src="../docs-site/public/og.png" alt="CodexCommander — OpenAI Codex와 Claude Code를 위한 범용 프로바이더 프록시" width="920">
</p>

<p align="center">
  <a href="../README.md">English</a> · <b>한국어</b> · <a href="README.zh-CN.md">简体中文</a> · <a href="README.ru.md">Русский</a> · <a href="README.ja.md">日本語</a> · 📖 <a href="../docs-site/src/content/docs/ko/getting-started/installation.md"><b>전체 문서 →</b></a>
</p>

Claude, Gemini, Grok, GLM, DeepSeek, Kimi, Qwen, Ollama 등 어떤 LLM이든 Codex에서 — 그리고 **Claude Code**에서도 — 사용하세요. 누군가 지원을 추가해 주길 기다릴 필요 없이.

CodexCommander는 Codex의 Responses API를 프로바이더가 쓰는 프로토콜로 변환해 주는 가벼운 로컬 프록시입니다. streaming, tool 호출, reasoning 토큰, 이미지까지 양방향으로 모두 동작합니다.
...
```mermaid
flowchart LR
  codex[Codex 세션<br/>CLI, App, SSH, 모바일] --> proxy[codexcommander]
...
[Bun](https://bun.sh)이 설치된 소스 체크아웃이 필요합니다. 세 플랫폼 모두 네이티브로 동작합니다 (Windows에서도 WSL 없이 사용 가능합니다). npm 패키지는 현재 게시되어 있지 않습니다.
...
bun run build:gui
bun run src/cli/index.ts init
bun run src/cli/index.ts start   # 백그라운드 실행에는 start 대신 service
```

http://localhost:10100에서 웹 대시보드를 열어 프로바이더, 모델, 계정을 설정하세요. `ccx gui`로 언제든지 다시 열 수 있습니다.
...
`ccx init`(대화형 CLI)이나 `~/.codexcommander/config.json` 직접 편집으로도 프로바이더를 추가할 수 있습니다.
...
`provider/` 접두사를 생략하면 CodexCommander는 기본 프로바이더로 라우팅하거나, 모델명 패턴으로 자동
...
컨트롤을 노출할 수 있습니다. CodexCommander는 프로바이더 config가 명시적으로 alias를 지정하지 않는 한
...
사용 가능 여부는 upstream preview gate를 따르며, CodexCommander는 계정/프로바이더가 제공할 때 쓸
routing/catalog metadata를 준비해 둡니다.

<p align="center">
  <img src="../assets/codex-app-picker.png" alt="codexcommander 라우팅 모델을 reasoning effort 선택기와 함께 보여주는 Codex App" width="480">
</p>
...
CodexCommander는 두 가지 동작을 분리해서 유지합니다:

- **기존 세션은 affinity를 유지합니다.** 스레드 id가 선택된 계정에 바인딩되어 이후 턴에서 재사용되므로,
  긴 요청이나 모바일/SSH 연결 세션이 같은 계정을 계속 사용합니다.
- **새 세션은 자동 라우팅됩니다.** 자동 전환이 켜져 있으면 CodexCommander는 5시간·주간·30일 사용량 중 가장
...
- **Claude에서도 어떤 LLM이든.** `ccx claude`로 Claude Code를 프록시에 연결해 실행할 수 있습니다. Claude 대시보드에는 Opus, Fable, Sonnet, Haiku를 관리하는 별도 Desktop 프로필과 드래그/키보드 조작, JSON 가져오기/내보내기도 있습니다.
...
- **알맞은 모델에 위임.** 대시보드나 config에서 최대 5개의 라우팅/네이티브 모델을 Codex 서브에이전트 추천(전체 허용 목록이 아님)으로 노출해, 복잡한 작업은 reasoning 모델로, 빠른 작업은 저렴한 모델로 보낼 수 있습니다. v2 멀티에이전트 표면(GPT-5.6 Sol/Terra)에서는 프록시가 간결한 위임 가이드를 주입합니다. 선호 서브에이전트 모델·effort(`injectionModel` / `injectionEffort`), 노출된 모델 로스터와 각 모델이 지원하는 effort 사다리, 그리고 크로스모델 `spawn_agent` 오버라이드를 적용하는 `fork_turns` 규칙까지. 알려진 제한: 네이티브 부모가 라우팅 자식을 스폰하면 작업 본문이 백엔드 암호화 상태로 도착해 유실될 수 있습니다([#92](https://github.com/pavelhov/CodexCommander/issues/92)) — 안정적인 크로스 프로바이더 위임에는 v1 표면을 쓰세요. 문구를 직접 쓰고 싶다면 `injectionPrompt`에 `{{model}}` / `{{effort}}` / `{{roster}}` 플레이스홀더를 넣으면 됩니다.
...
- **깔끔한 종료, 잔여물 제로.** `ccx stop`(또는 대시보드의 Stop 버튼)은 프록시를 종료하고, 설치된 백그라운드 서비스를 멈추며, Codex를 원래 설정으로 복원합니다. 이후 `codex`는 잔여 설정이나 좀비 프로세스 없이 이전과 똑같이 동작합니다.
...
그 외에 DeepSeek, Groq, OpenRouter, Together, Fireworks, Cerebras, Mistral, Hugging Face, NVIDIA NIM, MiniMax, Qwen Cloud, Tencent Cloud Coding Plan, SiliconFlow 등이 있습니다. 전체 목록은 `ccx init` 또는 [프로바이더 문서](../docs-site/src/content/docs/ko/reference/configuration/providers.md)에서 확인하세요.

## CLI

```bash
ccx init                       # 대화형 설정
ccx start [--port 10100]       # 프록시 시작; 포트가 사용 중이면 빈 포트로 자동 전환
ccx stop                       # 프록시 중지 + Codex 원래 설정 복원
ccx restore                    # 중지 없이 복원 (별칭: ccx eject)
ccx uninstall                  # service/shim/config 제거 + Codex 원본 복원
ccx ensure                     # 필요 시 시작 + Codex config/cache 갱신
ccx sync                       # 모델 갱신 + Codex에 재주입
ccx status                     # 프록시 실행 중인지 확인
ccx login <xai|anthropic|kimi> # OAuth 로그인
ccx logout <provider>          # 저장된 로그인 정보 삭제
ccx account <list|current|use> # 계정/API key pool 조회·전환 (마스킹; refresh/auto-switch/remove/add-key 포함)
ccx gui                        # 웹 대시보드 열기
ccx claude [args...]           # 프록시에 연결된 Claude Code 실행 (모델 디스커버리 켜짐)
ccx claude desktop             # Claude Desktop 4개 family 프로필 저장 및 적용
ccx codex-shim install         # codex 실행 시 `ccx ensure` 실행
ccx service [install|start|stop|status|uninstall]   # 백그라운드 서비스 설치/갱신/시작
```
...
```bash
ccx claude desktop apply                           # 현재 프로필 저장 및 적용
ccx claude desktop show [--json]                   # 라우트, family, 기본값 확인
ccx claude desktop move <route> <family> [--default]
ccx claude desktop default <family> <route|none>
ccx claude desktop export <path|->                 # -를 쓰면 stdout으로 JSON 출력
ccx claude desktop import <path> [--apply]         # 검증 후 저장, 선택적으로 바로 적용
```
...
비어 있지 않은 family에는 항상 기본값이 필요합니다. 적용하려면 명시적인 `apply` 명령을 사용합니다.

### 자동 시작: service vs shim

codexcommander에는 프록시를 자동 시작하는 두 가지 방법이 있습니다:
...
| **시점** | 로그인 후 항상 실행 | 온디맨드 — `codex` 실행 시 `ccx ensure` 실행 |
| **재시작** | 크래시 시 자동 재시작 | `codex` 호출마다 한 번 시작 |
| **Codex 업데이트** | 영향 없음 | 안정적으로 교체가 끝난 런처는 다음 일반 `ccx` 명령에서 복구 |
| **제거** | `ccx service uninstall` | `ccx codex-shim uninstall` |

항상 프록시를 켜두려면 **service** (개발 머신 권장), 가볍게 온디맨드로 쓰려면 **shim**을 사용하세요.

외부 Codex 업데이트가 설치된 shim을 덮어쓰면 다음 일반 `ccx` 명령이 안정화된 새 런처를 백업하고
shim을 복구합니다. 아직 변경 중인 런처는 건드리지 않고 이후 명령에서 다시 시도합니다. 복구 실패는
요청한 명령을 실패시키지 않고 경고만 출력하며, 수동 대체 명령은 `ccx codex-shim install`입니다.
자동 복구를 끄려면 `codexShimAutoRestore`를 `false`로 설정하거나 프로세스에
`CODEXCOMMANDER_CODEX_SHIM_AUTO_RESTORE=0`을 설정하세요.
shim 자동 시작은 기본으로 켜져 있으며 GUI 대시보드에서 끌 수 있습니다. 설정된 프록시 포트가 이미 사용
중이면 `ccx start`가 자동으로 다른 빈 로컬 포트를 고르고 Codex 설정도 그 포트로 갱신합니다.

### 삭제

로컬 상태를 삭제하려면 먼저 다음을 실행하세요:

```bash
ccx uninstall
```

`ccx uninstall`은 프록시 중지, 설치된 service 제거, Codex shim 제거, Codex config/catalog 원복, `~/.codexcommander` 삭제를 처리합니다.

## 설정

설정 파일은 `~/.codexcommander/config.json`에 저장됩니다. 파일이 깨진 경우(잘못된 JSON 등)
CodexCommander는 `config.json.invalid-<timestamp>`로 백업하고 경고를 출력한 뒤 기본값으로 시작합니다.
...
> 동작합니다 — codexcommander가 요청 전에 끝의 `[1m]` 접미사를 제거하기 때문입니다(OpenAI 호환 엔드포인트는
...
로컬 모델도 동작합니다. codexcommander를 머신에서 실행 중인 OpenAI 호환 서버로 향하게 하세요:
...
기본적으로 CodexCommander는 `127.0.0.1`(루프백)에 바인딩되며 별도 인증이 필요 없습니다.
`"hostname": "0.0.0.0"`으로 LAN에 노출할 경우, CodexCommander는 관리 API(`/api/*`)와 데이터 플레인
...
서비스를 설치할 때도 같은 셸에서 이 변수를 먼저 설정한 뒤 `ccx service install`을 실행해야 합니다.
...
모든 필드에 대한 자세한 내용은 **[설정 레퍼런스](../docs-site/src/content/docs/ko/reference/configuration/server.md)** 를 참고하세요.

## 문서

문서(설치, 프로바이더, 라우팅, sidecar, Codex 통합, CLI/설정 레퍼런스)는 [`docs-site/`](../docs-site)에 있습니다. 로컬에서 빌드할 수 있으며 이 저장소에는 게시 자동화가 포함되지 않습니다.

유지보수용 불변 조건은 [`structure/`](../structure)에, 유지 관리되는 기술·구현 노트는 [`docs/`](../docs)에 있습니다.
...
`bun run dev`는 `bun run dev:proxy`의 별칭입니다. 소스 체크아웃에서 프록시
...
**[기여하기](../docs-site/src/content/docs/ko/contributing.md)** 를 참고하세요.

## 면책 조항

CodexCommander는 독립적인 커뮤니티 프로젝트이며, **OpenAI, Anthropic 등 어떤 제공업체와도 제휴하거나 보증을 받지 않습니다.**

일부 제공업체 — 특히 Anthropic (Claude) — 는 서드파티 프록시를 통한 API 트래픽 라우팅 시 계정을 정지하거나 제한할 수 있습니다. **사용에 따른 책임은 본인에게 있습니다 (UAYOR).** 제공업체를 연결하기 전에 해당 서비스 약관에서 프록시 기반 접근이 허용되는지 확인하세요. codexcommander 유지보수자는 업스트림 제공업체의 계정 조치에 대해 책임을 지지 않습니다.
