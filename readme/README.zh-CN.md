...
<p align="center">
  <img src="../docs-site/public/og.png" alt="CodexCommander — 面向 OpenAI Codex 与 Claude Code 的通用 provider 代理" width="920">
</p>

<p align="center">
  <a href="../README.md">English</a> · <a href="README.ko.md">한국어</a> · <b>简体中文</b> · <a href="README.ru.md">Русский</a> · <a href="README.ja.md">日本語</a> · 📖 <a href="../docs-site/src/content/docs/zh-cn/getting-started/installation.md"><b>完整文档 →</b></a>
</p>

在 Codex 中 —— 以及在 **Claude Code** 中 —— 使用 Claude、Gemini、Grok、GLM、DeepSeek、Kimi、Qwen、Ollama 或任意其他 LLM，无需等待官方添加支持。

CodexCommander 是一个轻量级本地代理，把 Codex 的 Responses API 翻译成你的 provider 所讲的协议。streaming、tool 调用、reasoning token、图片 —— 全部双向工作。
...
```mermaid
flowchart LR
  codex[Codex 会话<br/>CLI, App, SSH, 移动端] --> proxy[codexcommander]
  proxy --> existing{已有线程?}
...
需要一个源码检出目录和已安装的 [Bun](https://bun.sh)。三个平台都原生运行（Windows 不需要 WSL）。npm 包目前尚未发布。
...
provider，或任意 OpenAI 兼容端点）、选择模型并管理账户。随时运行 `ccx gui` 可重新打开仪表板。
...
- **在 Claude 中也能使用任意 LLM。** `ccx claude` 可通过代理启动 Claude Code。Claude 仪表盘还提供独立的 Desktop 配置，可管理 Opus、Fable、Sonnet、Haiku 四个系列，并支持拖放、键盘操作和 JSON 导入/导出。
...
- **委派给合适的模型。** 在仪表盘或 config 中把最多 5 个路由/原生模型作为 Codex subagent 的推荐项（不是完整允许列表） —— 复杂任务交给 reasoning 模型，快速任务交给便宜模型。在 v2 多智能体表面（GPT-5.6 Sol/Terra）上，代理会注入精简的委派指引：首选子智能体模型与 effort（`injectionModel` / `injectionEffort`）、featured 模型清单及各自支持的 effort 阶梯，以及让跨模型 `spawn_agent` 覆盖得以应用的 `fork_turns` 规则。已知限制：原生父代理 spawn 路由子代理时，任务正文可能以后端加密形式到达而丢失（[#92](https://github.com/pavelhov/CodexCommander/issues/92)）—— 需要可靠的跨 provider 委派请使用 v1 表面。想自定义文案，可在 `injectionPrompt` 中使用 `{{model}}` / `{{effort}}` / `{{roster}}` 占位符。
...
- **干净退出，零残留。** `ccx stop`（或仪表盘的 Stop 按钮）会关闭代理、停止已安装的后台服务，并将 Codex 恢复为原始配置。之后 `codex` 就像从未安装过 codexcommander 一样工作 —— 无残留配置，无僵尸进程。
...
你也可以通过 `ccx init`（交互式 CLI）或直接编辑 `~/.codexcommander/config.json` 来添加 provider。
...
省略 `provider/` 前缀时，codexcommander 会路由到默认 provider，或根据模型名模式自动匹配（例如 `claude-*`
...
除非 provider config 明确设置 alias，codexcommander 会把 `xhigh` 与 `max` 保持为不同档位。`ultra` 与上游
...
preview gate 限制；codexcommander 只是准备好你的账户/provider 可访问时所需的路由和目录元数据。

<p align="center">
  <img src="../assets/codex-app-picker.png" alt="Codex App 展示 codexcommander 路由模型及 reasoning effort 选择器" width="480">
</p>
...
codexcommander 保持两种独立行为：

- **现有会话保持 affinity。** 线程 id 绑定到所选账户并在后续轮次复用，因此长请求或移动/SSH 连接的会话
  会继续使用同一账户。
- **新会话可自动路由。** 启用自动切换后，codexcommander 比较 5 小时、每周、30 天使用量中最热的配额窗口，
...
此外还有 DeepSeek、Groq、OpenRouter、Together、Fireworks、Cerebras、Mistral、Hugging Face、NVIDIA NIM、MiniMax、Qwen Cloud、腾讯云 Coding Plan、SiliconFlow 等等。完整列表可通过 `ccx init` 查看，或参阅 [provider 文档](../docs-site/src/content/docs/zh-cn/reference/configuration/providers.md)。

## CLI

```bash
ccx init                       # 交互式初始化
ccx start [--port 10100]       # 启动代理
ccx stop                       # 停止并恢复原生 Codex 配置
ccx restore                    # 仅恢复，不停止（别名：ccx eject）
ccx uninstall                  # 移除 service/shim/config 并恢复原生 Codex
ccx ensure                     # 按需启动 + 刷新 Codex config/cache
ccx sync                       # 刷新模型列表 + 重新注入 Codex
ccx status                     # 查看代理是否在运行
ccx login <xai|anthropic|kimi> # OAuth 登录
ccx logout <provider>          # 移除已保存的登录
ccx account <list|current|use> # 查看/切换账号与 API-key pool（脱敏；含 refresh/auto-switch/remove/add-key）
ccx gui                        # 打开 Web 仪表盘
ccx claude [args...]           # 启动接入代理的 Claude Code（模型发现已开启）
ccx claude desktop             # 保存并应用 Claude Desktop 四系列配置
ccx codex-shim install         # 运行 codex 时自动启动代理
ccx service [install|start|stop|status|uninstall]   # 安装/更新/启动后台服务
```
...
```bash
ccx claude desktop apply                           # 保存并应用当前配置
ccx claude desktop show [--json]                   # 查看路由、系列和默认值
ccx claude desktop move <route> <family> [--default]
ccx claude desktop default <family> <route|none>
ccx claude desktop export <path|->                 # 使用 - 将 JSON 输出到 stdout
ccx claude desktop import <path> [--apply]         # 验证后保存，可选择立即应用
```

`family` 可取 `opus`、`fable`、`sonnet`、`haiku`。非 Anthropic 路由会获得带有合成 2026 日期
槽位的稳定 Claude 格式别名；该日期是内部槽位，不是模型发布日期。真正的 Anthropic Claude
路由保留原始模型 id。`none` 只能用于空系列；非空系列始终需要一个默认值。应用时必须显式使用 `apply` 命令。

### 自动启动：service vs shim

codexcommander 提供两种自动启动代理的方式：
...
| **Codex 更新** | 不受影响 | 稳定完成的启动器替换会在下一条普通 `ccx` 命令中修复 |
| **移除** | `ccx service uninstall` | `ccx codex-shim uninstall` |

如需常驻代理，使用 **service**（推荐开发环境）。轻量按需启动使用 **shim**。

如果外部 Codex 更新覆盖了已安装的 shim，下一条普通 `ccx` 命令会备份已稳定的新启动器并恢复
shim。仍在变化的启动器不会被改动，而会在后续命令中重试。修复失败只会警告，不会让请求的命令
失败；手动备用命令为 `ccx codex-shim install`。若要关闭自动恢复，请将
`codexShimAutoRestore` 设为 `false`，或为进程设置
`CODEXCOMMANDER_CODEX_SHIM_AUTO_RESTORE=0`。
如果配置的代理端口已被占用，`ccx start` 会自动选择另一个空闲本地端口并更新 Codex 使用它。
...
`ccx uninstall` 会停止代理、移除已安装的 service、移除 Codex shim、恢复原生 Codex config/catalog，并删除 `~/.codexcommander`。

## 配置

配置文件路径：`~/.codexcommander/config.json`。
...
> codexcommander 会在发送请求前剥离末尾的 `[1m]` 后缀，因为 OpenAI 兼容端点会拒绝带方括号的 id（Z.AI 400 code
...
本地 provider 的 `apiKey` 通常留空。只要你的本地服务暴露了 OpenAI 兼容的 Chat Completions 端点，codexcommander 就能直接对接。

WebSocket 传输默认关闭。只有当你希望 Codex 使用 Responses WebSocket 而不是 HTTP/SSE 时，才需要设置 `"websockets": true`。

### 远程访问

默认情况下 codexcommander 绑定到 `127.0.0.1`（回环）且无需额外认证。
如果你设置 `"hostname": "0.0.0.0"` 把代理暴露到局域网，codexcommander 会要求一个 bearer token 来同时保护管理
...
绑定到非回环地址时若缺少该环境变量，代理会拒绝启动。若为局域网访问安装后台服务，请在 `ccx service install`
...
每个字段的详细说明参阅 **[配置参考](../docs-site/src/content/docs/zh-cn/reference/configuration/server.md)**。

## 文档

安装、provider 配置、路由、sidecar、Codex 集成和 CLI/配置参考位于 [`docs-site/`](../docs-site)。它们可以在本地构建；本仓库不包含发布自动化。

维护者不变式位于 [`structure/`](../structure)，持续维护的工程与实现笔记位于 [`docs/`](../docs)。
...
`bun run dev` 是 `bun run dev:proxy` 的别名。在源码检出中，代理 API 暴露 `/healthz`、
...
参阅 **[贡献指南](../docs-site/src/content/docs/zh-cn/contributing.md)**。

## 免责声明

CodexCommander 是一个独立的社区维护项目，**与 OpenAI、Anthropic 或任何其他提供商无关，也未获得其认可。**

某些提供商——尤其是 Anthropic (Claude)——可能会对通过第三方代理路由 API 流量的账户进行暂停或限制。**使用风险自负 (UAYOR)。** 在连接提供商之前，请查阅其服务条款以确认是否允许基于代理的访问。codexcommander 维护者不对上游提供商采取的任何账户操作承担责任。
