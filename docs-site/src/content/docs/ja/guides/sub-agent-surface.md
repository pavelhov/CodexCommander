...
- **サブエージェント** → **Agent Command Center**:
  ...
  - **Run Policy** は、エージェントプロトコル、優先ガイダンスモデルと effort、生成された子タスク用のグローバルフォールバック、ヘルス再確認間隔、スレッド制限、サブエージェント effort 上限、ロースターガイダンス、ネイティブ Codex デフォルト同期をステージングします。ポリシーの変更はロースターの変更とは別に保存します。
