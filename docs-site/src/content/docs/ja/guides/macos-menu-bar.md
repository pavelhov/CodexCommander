...
- **プロバイダークォータ** — OpenCodex が返す実際の 5 時間、週間、月間、またはプロバイダー
  固有のクレジット枠を、リセット時刻も含めてすべて表示します。欠けているデータは利用不可と
  表示され、使用量ゼロや無制限の容量として表示されることはありません。
...
コンパニオンは別のログインシステムを作成せず、macOS Keychain も使用しません。
...
この2つのスクリプトは Swift を直接呼び出すため、npm の依存関係をインストールする必要はありません。
