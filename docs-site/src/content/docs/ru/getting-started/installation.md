...
:::note
CodexCommander не удаляет вашу конфигурацию Codex. `ccx stop`, `ccx restore` и `ccx eject` удаляют
из `config.toml` только точный маршрут с маркером владения и восстанавливают native Codex.
Сгенерированные каталоги и кэши могут остаться, но native Codex на них не ссылается; задачи, история
и аутентификация не меняются.
:::
