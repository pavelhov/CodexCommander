...
прокси, не меняя жизненный цикл самого прокси. Route Back — явный переход ON: если существует
recovery journal, он удаляет только journal, доказанно устаревший под существующей координацией;
иначе Codex остаётся native/OFF:
