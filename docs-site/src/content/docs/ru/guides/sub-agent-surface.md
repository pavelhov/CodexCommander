...
| --- | --- |
| `{{model}}` | Настроенный `injectionModel` либо пустая строка |
...
убирает ростер, а не обрезает ядро инструкций для spawn. Guidance включается только тогда, когда
разрешается предпочитаемая модель, допустимый ростер или fallback chain. Пользовательский prompt
не обходит это условие.
...
- **Models** → **Current behavior** → **Collaboration**: выберите **Classic v1**, **Automatic** (base) или **Concurrent v2**.
