...
- **Models** → **Current behavior** → **Collaboration**: выберите **Classic v1**, **Следовать настройкам Codex** (base) или **Concurrent v2**.
...
вместе с корневым, а V1 — дочерние потоки. Протокол и лимит применяются к новым сессиям, подсказки
и fallback — к будущим дочерним задачам; страница сообщает, если у запущенного Codex app-server
остался устаревший каталог.
