...
- **Subagents** → **Agent Command Center**:
  - **Настроенный состав** выбирает и упорядочивает пять переопределений моделей, первыми объявляемых
...
после сохранения проверьте состояние активации. Если каталог на диске и управляемая маршрутизация уже
актуальны, а устарел только работающий воркер, полностью завершите ChatGPT, снова откройте его и затем
начните новую задачу. Это рекомендуемый и наиболее надёжный путь. Если каталог имеет состояние pending
или unknown либо маршрутизация не внедрена, сначала используйте **Применить к Codex**; одного перезапуска
ChatGPT недостаточно. При внешней или неизвестной маршрутизации Apply остаётся недоступен, а файлы
маршрутизации и каталога не меняются; при отключённой интеграции ростер остаётся только в
CodexCommander. Для уже согласованных каталога и маршрутизации с устаревшим воркером защищённые
**Принудительно перезапустить воркеры**, `/api/codex-catalog/apply` и `ccx sync --restart-codex`
остаются расширенными запасными вариантами; они могут вызвать в ChatGPT сообщение
**“stopped unexpectedly”**. Подсказки и fallback применяются к будущим дочерним задачам. Изменение
только доставки V2 требует лишь новой задачи и не делает каталог dirty.
...
| `/api/codex-catalog/apply` | Согласовать pending-каталог или ещё не внедрённую управляемую маршрутизацию, а затем при необходимости принудительно перезапустить только подтверждённые устаревшие воркеры после согласия на прерывание. Когда каталог и управляемая маршрутизация уже актуальны, а устарел только воркер, это расширенный запасной вариант, после которого ChatGPT может показать **“stopped unexpectedly”**; browser-вызов доступен только после подтверждённого запуска через `ccx gui` или приложение меню. |
...
proxy. Новый task или fork не является границей перезагрузки. Если каталог на диске и маршрутизация
уже актуальны, а устарел только воркер, полностью завершите ChatGPT, снова откройте его и начните новую
задачу. Если каталог pending/unknown или маршрутизация не внедрена, сначала выберите **Применить к Codex**:
одного ручного перезапуска недостаточно.
...
App host перечитать каталог. Save и `ccx sync` не прерывают работу. Когда каталог и управляемая
маршрутизация актуальны, а устарел только воркер, полностью завершите ChatGPT, снова откройте его и
затем начните новую задачу. Если каталог имеет состояние pending/unknown или управляемая маршрутизация
не внедрена, сначала используйте **Применить к Codex**: перезапуск сам по себе этого не делает.
Защищённое **Принудительно перезапустить воркеры** и `ccx sync --restart-codex` остаются расширенными
запасными вариантами и могут привести к сообщению ChatGPT **“stopped unexpectedly”**. Автоматического
Apply, очереди ожидания простоя и сохраняемого снимка pending намеренно нет.
