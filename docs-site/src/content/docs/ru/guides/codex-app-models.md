...
`ccx start` и `ccx sync` подключают общий конфиг и каталог Codex к прокси. `ccx init` может сделать
это только через уже запущенный прокси, подтверждённый защищённой runtime-записью; иначе Codex
остаётся native до явного Start. Подробности о внедрении конфигурации, синхронизации каталога,
shim'ах, fallback с WebSocket и механике восстановления см. в [Интеграции с Codex](/guides/codex-integration/).
...
Настроенный состав отделён от выбора **Sub-agent delegation** в дашборде. Он только
