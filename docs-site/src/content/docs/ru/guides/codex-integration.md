...
`ccx start` и `ccx sync` вызывают injector. `ccx init` входит в тот же путь только через уже
запущенный прокси, подтверждённый защищённой runtime-записью текущего home; иначе Codex остаётся
native до явного Start. На loopback-привязке по умолчанию injector сохраняет встроенный id
провайдера Codex `openai` и направляет его на CodexCommander:
...
CodexCommander не запирает вас внутри себя. **`ccx stop` — одна команда, которая безопасно возвращает
Codex к нативной маршрутизации и только затем останавливает CodexCommander.** Она сохраняет
интеграцию как OFF, удаляет из `$CODEX_HOME/config.toml` только точный маршрут, принадлежащий
CodexCommander, проверяет native routing и затем останавливает фоновую службу и прокси. Если проверка
не удалась, оба продолжают работать. Задачи, история, rollout и аутентификация не меняются.
Сгенерированные каталоги и кэши могут остаться, но native Codex больше на них не ссылается:
