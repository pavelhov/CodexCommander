---
title: Claude Code
description: Используйте любую маршрутизируемую модель из Claude Code — CodexCommander обслуживает Anthropic Messages API и обнаружение моделей шлюза на одном и том же порту.
---
...
`ccx claude` убеждается, что прокси запущен, а затем запускает Claude Code с уже настроенным окружением:
...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` / `ANTHROPIC_SMALL_FAST_MODEL` | `claudeCode.smallFastModel`, если настроен |

Переменные, которые вы экспортируете сами, всегда имеют приоритет. Дополнительные аргументы передаются как есть: `ccx claude -p "hello"`.

## Интеграция с системным окружением (macOS)

Когда `claudeCode.systemEnv` установлен в `true` (по умолчанию: **выключено**), `ccx start`
...
обычные команды `claude` через прокси без обёртки `ccx claude`. Уже открытые оболочки не
затрагиваются — их нужно открыть заново.

`ccx stop` и остановка прокси **снимают внедрённые ключи** (прежние значения не восстанавливаются —
удаляются только ключи, внедрённые CodexCommander). Прокси также записывает `~/.codexcommander/claude-env.sh`;
`ccx start` устанавливает source-хук в `.zshrc`, который загружает этот файл автоматически.

Отключить можно параметром `claudeCode.systemEnv: false` в конфигурации или переключателем в GUI.
Функция доступна только на macOS; на других платформах используйте `ccx claude`.
...
`accept-encoding`, `x-codexcommander-api-key` и `origin` удаляются перед пересылкой. Все остальные
...
что предупреждение «claude.ai connectors are disabled» с `ccx claude` больше не появляется.
...
начинающиеся с `claude` или `anthropic`, CodexCommander публикует маршрутизируемые модели как
стабильные обратимые алиасы:

| Интерфейс | Формат | Пример |
| --- | --- | --- |
| Claude Code CLI | `claude-ccx2-<provider>--<model>` (plain) или `claude-ccx2-…` (escaped) | `claude-ccx2-native--gpt-5.6-sol` |
...
текущих семейства разрешаются через реестр алиасов запущенного процесса.

Если нижний селектор Claude Desktop не переключает модель в уже запущенном 3P-диалоге,
используйте `/model <id>` внутри этого диалога. CodexCommander не видит состояние селектора и
...
`native`. Текущая кодировка `claude-ccx2-…` экранирует `/` как `~s`, а `~` как `~t`,
например `openrouter/anthropic/claude-opus-4-8` →
`claude-ccx2-openrouter--anthropic~sclaude-opus-4-8`. Маршруты, которые невозможно выразить читаемой формой, откатываются на
...
`effectiveModelEnv` вычисляет два вспомогательных слота, внедряемых `ccx claude`, системным
окружением и shell-файлом: `ANTHROPIC_DEFAULT_HAIKU_MODEL` и `ANTHROPIC_SMALL_FAST_MODEL`.
Оба получают значение `claudeCode.smallFastModel`.

Если `smallFastModel` отсутствует, CodexCommander оставляет обе переменные вспомогательной модели незаданными. Затем Claude Code выбирает нативную вспомогательную модель, что может привести к расходам у нативного провайдера.

## Агенты из ростера (injectAgents)

`ccx claude` (и демон системного окружения) синхронизирует ваш ростер избранных подагентов
(вкладка Subagents, до 5 моделей) плюс `ccx-self` в `~/.claude/agents/ccx-*.md`.

- **`ccx-self`** закрепляет модель по умолчанию из селектора `/model`; если её нет, файл не
  создаётся. Наследование модели НЕ используется.
- В теле каждого агента есть директива `<!-- ccx-route: <model> -->` — по ней прокси закрепляет
...
- Перезаписываются или удаляются только проверенные по маркеру файлы `ccx-*.md`, содержащие
  `generated-by: codexcommander`; ваши собственные агенты никогда не затрагиваются.
...
Диспетчеризация: `subagent_type: "ccx-gpt-5-6-sol"`. Цели с поддержкой 1M автоматически получают `[1m]`.
...
этом пакете не обучались, поэтому по умолчанию CodexCommander подменяет содержимое скилла короткой
...
изображений. CodexCommander закрывает эти пробелы до того, как ответит основная модель:
...
Явно указанный `backend` всегда имеет приоритет. Если он не задан, CodexCommander выбирает
...
CodexCommander не заимствует втихую учётные данные ChatGPT и не переключает бэкенды. Бэкенд OpenAI
...
Журнал запросов показывает разобранное запрошенное значение и, когда маршрутизированный адаптер
предоставляет исходящую диагностику, точное сериализованное значение как
**Запрошено → отправлено**. «Отправлено» подтверждает, что CodexCommander поместил значение в wire,
но не то, что вышестоящий provider его применил.
...
Захват входящих запросов управляется командой `ccx debug claude on|off|status|reset`, переменной
`CCX_CLAUDE_DEBUG=1` или запросом `PUT /api/debug {"claude": true}`.
...
- Быстрый старт (`ccx claude`) и блок ручной настройки окружения
...
`web_search_tool_result`, включая `usage.server_tool_use.web_search_requests`. Если поиск выполняется,
но Claude Code всё равно насчитывает ноль, проверьте, что запущенный процесс CodexCommander пересобран из текущего checkout.
...
`ANTHROPIC_AUTH_TOKEN`. `ccx claude` намеренно НЕ устанавливает `ANTHROPIC_API_KEY`; если вы
экспортировали его сами, снимите переменную. `ccx claude` внедряет `ANTHROPIC_BASE_URL`,
...
`CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` (с `ccx claude` — автоматически). Запустите
`ccx claude`, чтобы обновить кеш моделей шлюза в `~/.claude/cache/gateway-models.json`.
Проверьте, что `claudeCode.enabled` не равен `false`.

**Окружение после смены порта** — если порт прокси изменился, в уже открытых оболочках
может остаться прежний `ANTHROPIC_BASE_URL`. Откройте новый терминал или повторно запустите
`ccx claude`.
...
для маршрутизируемых моделей CodexCommander по умолчанию подставляет заглушку
(`blockedSkills: ["claude-api"]`).

**Подагент отправляется не в ту модель** — агенты из ростера (`ccx-*`) используют директивы
`<!-- ccx-route: ... -->`, а не аргумент `model` инструмента Agent. Убедитесь, что директива
