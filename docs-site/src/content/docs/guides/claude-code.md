---
title: Claude Code
description: Use any routed model from Claude Code — CodexCommander serves the Anthropic Messages API and gateway model discovery on the same port.
---

CodexCommander serves `POST /v1/messages` (plus `count_tokens`) alongside `/v1/responses`, so Claude
...
You can log in multiple Claude accounts via the Providers dashboard (`ccx login anthropic` /
...
`ccx claude` ensures the proxy is running, then launches Claude Code with the environment wired:
...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` / `ANTHROPIC_SMALL_FAST_MODEL` | `claudeCode.smallFastModel` when configured |

Variables you export yourself always win. Extra arguments pass through: `ccx claude -p "hello"`.
...
silently disabled a healthy claude.ai subscription in favour of API billing. `ccx claude` now
...
depends on something CodexCommander can look up, so by default it does.

Leave **Auth mode** on **Auto** (the default) in **Claude → Claude Code** and CodexCommander
...
next `ccx claude` with nothing to reconfigure.
...
`claude` launched outside `ccx` behaves the same way. That file is a snapshot refreshed when
the proxy starts or you save settings, while `ccx claude` always resolves live.
...
```bash
ccx claude desktop apply
ccx claude desktop show [--json]
ccx claude desktop move <route> <opus|fable|sonnet|haiku> [--default]
ccx claude desktop default <opus|fable|sonnet|haiku> <route|none>
ccx claude desktop export <path|->
ccx claude desktop import <path> [--apply]
```

`ccx claude desktop apply` writes the current profile to Claude Desktop. `show` gives a
...
`CODEXCOMMANDER_CLAUDE_DESKTOP_CONFIG_DIR` for an explicit library override or
`CLAUDE_USER_DATA_DIR` for an alternate Desktop user-data root.
...
provider or model it calls.

## System Environment Integration

When `claudeCode.systemEnv` is set to `true` (default: **off**), `ccx start` uses `launchctl setenv`
...
requiring the `ccx claude` wrapper. Already-open shells are unaffected and must be reopened.

`ccx stop` and proxy shutdown **unset the injected keys** (it does not restore previous values —
only the keys CodexCommander injected are removed). The proxy also writes `~/.codexcommander/claude-env.sh`;
`ccx start` installs a `.zshrc` source hook that loads it automatically.

Disable with `claudeCode.systemEnv: false` in the configuration or with the GUI toggle. This
feature is macOS-only; on other platforms, use `ccx claude`.
...
`x-codexcommander-api-key`, and `origin` are stripped before forwarding. All other headers (including
...
"claude.ai connectors are disabled" warning no longer appears with `ccx claude`.
...
with `claude` or `anthropic`, CodexCommander exposes routed models as stable, reversible aliases:

| Surface | Format | Example |
| --- | --- | --- |
| Claude Code CLI | `claude-ccx2-<provider>--<model>` (plain) or `claude-ccx2-…` (escaped) | `claude-ccx2-native--gpt-5.6-sol` |
...
Both current families resolve through the running alias registry.
...
canonical ids. The synthetic 2026 date is an internal slot, not a release date.

If Claude Desktop's footer picker does not change the model for an already-running 3P
conversation, use `/model <id>` in that conversation. CodexCommander cannot observe picker state; it
...
The current `claude-ccx2-…` encoding escapes `/` as `~s` and `~` as `~t`, e.g.
`openrouter/anthropic/claude-opus-4-8` → `claude-ccx2-openrouter--anthropic~sclaude-opus-4-8`.
Routes that the readable form cannot express fall back to the hashed alias. Model ids MAY contain
`--` (resolution splits on the first `--` only); native slugs containing `--` fall back to the
hashed form.
...
`effectiveModelEnv` computes the two helper slots injected by `ccx claude`, system env, and the
shell file: `ANTHROPIC_DEFAULT_HAIKU_MODEL` and `ANTHROPIC_SMALL_FAST_MODEL`. Both receive
`claudeCode.smallFastModel`.

When `smallFastModel` is absent, CodexCommander leaves both helper variables unset; Claude Code then
chooses its native helper model, which may incur native-provider charges.

## Roster agents (injectAgents)

`ccx claude` (and the system-env daemon) syncs your featured subagent roster (Subagents tab,
up to 5 models) plus `ccx-self` into `~/.claude/agents/ccx-*.md`.

- **`ccx-self`** pins your `/model` picker default; it is omitted when no picker default exists.
  It does NOT use model inheritance.
- Each agent body contains an `<!-- ccx-route: <model> -->` directive — the proxy uses this to
...
- Only marker-verified `ccx-*.md` files containing `generated-by: codexcommander` are ever
...
Dispatch: `subagent_type: "ccx-gpt-5-6-sol"`. 1M-capable targets carry `[1m]` automatically.
...
default CodexCommander replaces the skill's content with a short stub on **routed** requests. Native
...
An explicit `backend` always wins. When it is omitted, CodexCommander selects `anthropic` if a usable
stored Anthropic OAuth account exists; otherwise it selects `openai`. Explicitly selecting
`anthropic` without a usable credential **fails closed**: CodexCommander does not silently borrow
...
`ccx debug claude on|off|status|reset`, `CCX_CLAUDE_DEBUG=1`, or `PUT /api/debug {"claude": true}`
...
- Quickstart (`ccx claude`) and manual env block
...
including `usage.server_tool_use.web_search_requests`. If a search completes but Claude Code still
counts zero, verify that the running CodexCommander process was rebuilt from the current checkout.
...
in your shell. `ccx claude` deliberately does NOT set `ANTHROPIC_API_KEY`; if you have it exported,
unset it. `ccx claude` injects `ANTHROPIC_BASE_URL`, discovery, auto-context, and configured model slots — but never `ANTHROPIC_API_KEY`.

**Models not showing in /model picker** — Verify `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` is
set (automatic with `ccx claude`). Run `ccx claude` to refresh the gateway model cache at
...
`ANTHROPIC_BASE_URL`. Open a new terminal, or re-run `ccx claude`.
...
on Claude model mentions. This is normal for native passthrough; on routed models, CodexCommander stubs
it by default (`blockedSkills: ["claude-api"]`).

**Subagent dispatches to wrong model** — Roster agents (`ccx-*`) use `<!-- ccx-route: ... -->`
