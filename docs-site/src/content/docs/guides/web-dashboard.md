...
| **Subagents** | Open the **Agent Command Center** to choose and order up to five featured `spawn_agent` suggestions, add optional per-model advisory guidance, search the current catalog, configure Run Policy, and install the optional advisory Codex delegation setup. Those five are suggestions, not an exhaustive native spawn allowlist. Save sends the ordered `{ roster: [{ model, guidance? }] }` form. Saved entries that are not advertised are reported explicitly. Its status distinguishes saved configuration, the generated on-disk catalog, and the roster loaded by current Codex workers. |
...
current collaboration tool contract and live CodexCommander guidance before delegating. Featured
models are suggestions, not an exhaustive allowlist. Those live
...
| `GET` / `PUT` / `PATCH /api/subagent-models` | Read or set up to five ordered roster objects. GET keeps `chosen: string[]` and adds `roster`; Save sends `{ roster }`. Guidance-only PATCH writes one row without replacing concurrent roster changes. Legacy `{ models }` writes preserve matching guidance. |
