...
- **중지·업데이트·콜드 스타트 후 네이티브 모델만 표시됨** — OpenCodex를 다시 열거나
  <code>ocx sync</code>를 실행하세요. 시작 시 공급자 검색 결과가 일시적으로 비어 있어도 보호된 마지막
  정상 카탈로그에서 아직 구성된 라우팅 모델을 복원합니다. 복원 후에도 ChatGPT가 이전 카탈로그를
  사용 중이라고 표시되면 ChatGPT를 한 번 재시작하세요. 컴패니언이 자동으로 종료하지는 않습니다.
