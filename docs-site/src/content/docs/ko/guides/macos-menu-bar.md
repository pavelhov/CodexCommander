...
두 라우팅 작업 중 하나를 선택하면 헤더 바로 아래에 상태 카드가 즉시 표시됩니다. 카드에는 스피너, 경과 시간,
그리고 실제 처리 단계인 **Changing route**와 **Confirming route**가 표시됩니다. 작업 중에는 라우팅 및 수명
주기 버튼이 비활성화되고, 대기 중에는 현재 활성 라우팅으로 전환하는 버튼이 비활성화됩니다. 진행 상태는 작업이
끝날 때까지 유지되며 최종 성공 또는 오류는 **Dismiss**를 선택하거나 다른 작업을 시작할 때까지 사라지지 않습니다.

**Restore Native Codex** 또는 **Route Codex Through Proxy**가 성공하면 ChatGPT를 완전히 종료하고 다시 연
뒤 새 작업을 시작하세요. 저장된 라우팅은 확인되었지만 이미 실행 중인 ChatGPT/Codex 호스트는 이전 라우팅을
계속 보유할 수 있습니다.
...
`codexcommander-journal.json`은 별도의 라우팅 설정이 아니라 보호된 복구 체크포인트입니다. 중단 후
CodexCommander가 쓴 정확한 설정과 이후의 사용자 편집을 구분하는 데 사용됩니다. 체크포인트가 검증된 실행 중
프록시에 속하고 profile 증거가 일치하며 관리되는 라우팅이 그대로 유지되면 **Route Codex Through Proxy**를
반복해도 안전한 no-op으로 처리됩니다. sync 이후의 관련 없는 Codex 환경설정 변경은 허용됩니다. 라우팅
소유권이 변경되었거나 custom 또는 모호하거나 안전하게 증명할 수 없으면 CodexCommander는 추측하지 않고 기존
라우팅을 그대로 둡니다. journal을 수동으로 삭제하거나 편집하지 마세요.
