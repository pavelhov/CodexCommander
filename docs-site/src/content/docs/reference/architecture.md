---
title: Architecture
description: CodexCommander internals — module map, the AdapterEvent bridge, the request parser, and caching.
---

CodexCommander is a single Bun process. A request enters as OpenAI Responses, is normalized to an internal
...
Three entry files are thin module facades: `codex/catalog.ts` exports
...
- **Messages** — `input` items become a normalized `CodexCommanderMessage[]`: user / developer / assistant /
...
requires `CODEXCOMMANDER_API_AUTH_TOKEN` for both `/api/*` and `/v1/*` when the proxy binds beyond
...
(`oauth/health.ts`) feeds `ccx status`, `ccx doctor`, the management API, and the dashboard.
...
and is not persisted across restarts. Request usage is normalized to `CodexCommanderUsage`, surfaced in
...
WebSocket upgrade while `websockets` is `false`, CodexCommander returns `426 upgrade_required`; Codex then
...
`CodexCommanderContentPart` (text / image), `CodexCommanderToolCall`, `CodexCommanderTool`, `AdapterEvent`, and the config types
(`CodexCommanderConfig`, `CodexCommanderProviderConfig`). Two helpers are widely used: `namespacedToolName()` and
