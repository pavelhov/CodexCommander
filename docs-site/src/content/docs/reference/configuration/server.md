...
Direct Grok generations accept a bounded `Idempotency-Key` or `X-Client-Request-Id`. Exact retries
with an explicit identifier replay the bounded private local result for up to 24 hours. Under result
storage pressure an older result can become a non-retryable `artifact_unavailable` tombstone without
repeating the paid POST. Without either header,
the same authenticated principal and exact request body use a 10-minute safety window; use a fresh
explicit identifier to deliberately regenerate an identical body.
