...
3. **Authenticate** — for an `oauth` provider, opencodex resolves the current access token as the
   bearer key and follows its credential owner: OpenCodex-owned credentials refresh automatically,
