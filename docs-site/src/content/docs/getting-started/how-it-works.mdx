...
`gpt-5.4-mini` as Codex's featured sub-agent suggestions. The dashboard can reorder or replace up to
five entries with native or routed models; those five are suggestions, not an exhaustive
`spawn_agent` allowlist. V1 requests receive only Codex's existing proactive delegation text at
