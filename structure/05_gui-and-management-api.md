...
CLI rather than being copied into Application Support. A released bundle instead prefers its own
`Contents/Resources/runtime`; neither path shells through an ambient `ocx`. On launch the app runs an
...
It never restarts the OpenCodex proxy or closes the menu app. The confirmation reports fresh active
request evidence when available and still warns that an answer may be interrupted; zero or unknown
activity is not presented as proof of idleness. Release one offers **Apply Now** and **Later**, not an
...
never disappears merely because its report is absent. The additive availability contract gives the
companion and Providers workspace the same auth/readiness result in the same refresh: linked Grok or
Kimi CLI credentials can say that the local CLI login needs refresh, rejected credentials can ask for
sign-in, and transport failures stay distinct. Older proxies that omit the field retain the generic
