...
| GUI session | A confirmed local-app launch, server-memory-backed and origin-bound; its token, CSRF token, origin, and absolute expiry are mirrored in browser `sessionStorage` | Full dashboard methods for up to eight hours; catalog Apply and managed delegation writes remain confirmed-session-only |
...
The raw admin principal remains capable of ordinary management API mutations. Two narrow browser
exceptions accept only a confirmed GUI session: catalog Apply, and install/update/remove of the
managed Codex delegation setup. The native companion and `ccx sync --restart-codex` keep their fixed
non-browser catalog flows; delegation setup instead offers a read-only manual copy fallback.
...
| Subagents | Read/write the featured `subagentModels` roster, capped at five ordered entries. Disk reads accept legacy `string[]` or `{ model, guidance? }[]`, while the canonical in-memory and newly written form is objects; after the first object write, older binaries that only read strings fail on that config. `GET /api/subagent-models` keeps `chosen: string[]` and adds the ordered `roster` objects. `PUT` accepts exactly one of `roster` or legacy `models`; the latter preserves existing guidance for models that remain in the list. Per-row guidance is optional sanitized operator text (empty omitted, max 160 Unicode code points), advisory and untrusted rather than an effort/quota/role/fallback control. The GUI Save action sends `{ roster }`. On eligible V2 turns, every accepted guidance annotation that survives compatibility filters is included in the built-in live developer text without an aggregate character budget; it is not copied into managed skill/`AGENTS.md` artifacts. `GET/PUT /api/injection-model` manages the shared delegation model/effort selection, the independent CodexCommander guidance switch, and the default-off `syncCodexSubagentDefaults` opt-in for native Codex subagent defaults. `GET /api/codex-catalog/status` exposes desired configuration, deterministic on-disk catalog evidence, and current worker activation evidence; `POST /api/codex-catalog/apply` is an authenticated, CSRF-protected, explicit interruption action accepting only `{ expectedDesiredRevision, confirmInterrupt: true }`. It accepts only the confirmed GUI session created by the exchanged single-use launch ticket; the raw admin token is mint authority, not direct browser-Apply authority. CLI and native-companion compatibility remains on their narrow non-browser flows. No caller-supplied PID, command, or path is accepted; stale targets are exact current-user identities revalidated before `SIGTERM`, unknown identity blocks, and busy work returns `503` plus `Retry-After`. The roster GET/PUT and `POST /api/sync` include the same additive activation observation. When CodexCommander owns the active Codex routing, native `[agents]` defaults apply to newly created Codex tasks after sync/restart; those defaults still use only `injectionModel` and `injectionEffort`, external user-managed provider configs remain untouched, and defaults do not cause delegation. Existing user-owned defaults are preserved rather than overwritten. PUT is partial-update: absent keys are unchanged, `null` clears, and non-object bodies are rejected with 400 before field validation. `syncCodexSubagentDefaults: true` requires a nonblank `model` and a supported Codex reasoning effort when effort is set; clearing `model` (null/empty) always clears effort and disables native-default sync even when the stored effort was invalid. The separate `/api/codex-delegation` resource manages only the advisory skill and bounded global `AGENTS.md` block described below; it does not change any setting in this row. |
...
### Managed Codex delegation resource

`src/server/management/delegation-routes.ts` owns `/api/codex-delegation`. Authenticated `GET` is
read-only and is available to either a raw admin principal or a confirmed GUI session. `PUT` and
`DELETE` require the confirmed GUI principal produced by the launch-ticket exchange; the global
management gate must also validate the exact origin and CSRF evidence. A raw admin principal receives
403 before the body is consumed. `PUT` accepts exactly `{ "mode": "balanced" }` or
`{ "mode": "orchestrator" }`, while `DELETE` accepts no request body.

Every response is `Cache-Control: no-store`. The projection exposes only the symbolic paths
`$HOME/.agents/skills/codexcommander-delegation/SKILL.md` and `$CODEX_HOME/AGENTS.md`, fixed state and
reason enums, and canonical packaged previews/copy instructions. It never serializes an inspected
absolute path, existing `AGENTS.md` or override contents, credentials, account identifiers, or a
caller-provided prompt. Foreign artifacts project as `ownership_conflict`; unsafe artifacts project
as `unsafe_path`.

The dashboard flow is: select Balanced or Orchestrator, preview both exact managed artifacts, confirm
Install/Update/Repair (or Change mode), then start a new Codex task. Remove setup has a separate
confirmation and uninstalls both managed artifacts. The manual copy disclosure is a fallback only
when the local installer is unavailable. Structural conflicts and unsafe paths disable automatic
mutation rather than offering a force option; an active `AGENTS.override.md` is reported as shadowing
the installed block rather than as Ready.

This resource is not part of `/api/v2`, the `subagentDeveloperInstructions` injection path, native
`[agents]` defaults, roster injection, or catalog lifecycle. It mutates no Codex config, does not
restart workers, and does not replace the proxy. The installed skill remains advisory: it consults
the live collaboration tools and roster, contains no model ids, and remains subordinate to user and
repository instructions. Codex loads the global block once per run, so mutation never changes the
instructions of the current task.
...
retains its ordinary management authorization and behavior. Management projections and the
provider-visible media tool replay contract remain path-free and secret-free.
