...
describe("vision sidecar fallback (issue #88, end-to-end)", () => {
  ...
  test("type-less and plaintext encrypted-content user messages share the early consent gate", async () => {
    ...
    const server = startServer(0);
    try {
      const blocked = await postResponses(server.url, {
        model: "passthrough/model",
        input: [{
          role: "user",
          content: [{
      ...
      expect(await blocked.json()).toMatchObject({ error: { code: "video_confirmation_required" } });
      expect(upstreamHits).toBe(0);
      ...
      expect(upstreamBodies[1]).not.toContain('"name":"video_gen"');
    } finally {
      await server.stop(true);
    }
  });
