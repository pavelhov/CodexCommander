...
describe("callXaiImages", () => {
  ...
  test("complete OAuth 401 returns needs_auth without refreshing or replaying the billable POST", async () => {
    const oauthBinding = { ...BINDING, authSource: "subscription_oauth" } as const;
    const snapshot = {
      provider: "xai",
      accountId: "internal-slot",
    ...
    const lease = {
      ...
      refreshAfterRejectedOAuth: async () => {
        refreshes += 1;
        return { bearer: "fresh-access", oauthSnapshot: { ...snapshot, accessToken: "fresh-access" } };
      },
    };
    const fetchFn = (async () => {
      sends += 1;
      return sends === 1
        ...
        : new Response(JSON.stringify({ data: [{ b64_json: "unexpected-replay" }] }));
    }) as typeof fetch;

    await expect(callXaiImages(
      { prompt: "no paid replay" }, oauthBinding, undefined, 5_000, { lease, fetchFn },
