...
describe("Responses auxiliary video intent", () => {
  ...
  test.each([
    ...
    "Create videos showing the sunrise and sunset.",
    "Make 2 clips of a fox in the snow.",
  ])("requires confirmation for unsupported multiple video outputs: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input })).toEqual({ state: "confirmation_required" });
  });
  ...
  ])("routes negated or cancelled video wording as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).toBe("none");
  });
  ...
  ])("routes quoted video wording as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).toBe("none");
  });
  ...
  test.each([
    ...
    "Review this code:\n~~~text\nCreate a video of a fox.\n~~~",
    "Review this code:\n    Create a video of a fox.",
  ])("routes fenced or indented Markdown discussion as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).toBe("none");
  });
  ...
  ])("routes list-contained examples as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).toBe("none");
  });
  ...
  ])("routes non-video compound objects as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).toBe("none");
  });
  ...
  ])("routes hypothetical video discussion as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).toBe("none");
  });

  test.each([
    "Could you create a video of a fox?",
    "Would you generate a video of a paper boat?",
  ])("keeps polite modal video requests behind confirmation: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input })).toEqual({ state: "confirmation_required" });
  });

  test.each([
    "List supported video resolutions.",
    "Tell me how video generation works.",
  ])("routes clearly informational video requests as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input })).toEqual({ state: "none" });
  });

  test("keeps an underspecified generation question behind confirmation", () => {
    expect(deriveCurrentUserVideoIntent({ input: "Create a video of a fox?" }))
  ...
  test.each([
    ...
    "text-to-video",
  ])("keeps malformed or underspecified video text behind confirmation: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input })).toEqual({ state: "confirmation_required" });
  });
  ...
  ])("fails closed for malformed or unsupported Unicode: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input })).toEqual({ state: "confirmation_required" });
  });
  ...
  test.each([
    ...
    "Create a video of a fox ❎.",
    "Create a video of a fox 🆖.",
  ])("never promotes malformed or mixed video wording into paid generation: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).not.toBe("explicit");
  });
  ...
  ])("routes analysis-only or non-executable wording as normal chat: %s", input => {
    expect(deriveCurrentUserVideoIntent({ input }).state).toBe("none");
  });
});
