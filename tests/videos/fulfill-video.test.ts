...
describe("parseVideoCallArgs", () => {
  ...
  test("accepts legacy string input as a prompt alias", () => {
    expect(parseVideoCallArgs(JSON.stringify({ input: " world " })))
      .toMatchObject({ ok: true, prompt: "world" });
  });

  test("rejects conflicting prompt and input aliases", () => {
    expect(parseVideoCallArgs(JSON.stringify({ prompt: "hello", input: "world" })))
      .toEqual({ ok: false, error: "prompt and input cannot both be supplied" });
    expect(parseVideoCallArgs(JSON.stringify({ input: 42 })))
      .toEqual({ ok: false, error: "input must be a string" });
  });
