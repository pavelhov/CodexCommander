...
export default function CodexDelegationSetupCard({ delegationSetup }: { delegationSetup: CodexDelegationSetupController }) {
  ...
  useEffect(() => { const timer = setTimeout(() => { if (typeof document === "undefined") return; if (previewOpen) (previewApply ? previewConfirmRef.current : document.querySelector<HTMLButtonElement>(".swi-delegation-dialog button"))?.focus(); if (removeOpen) document.querySelector<HTMLButtonElement>('[role="alertdialog"] button')?.focus(); }, 0); return () => clearTimeout(timer); }, [previewApply, previewOpen, removeOpen]);
