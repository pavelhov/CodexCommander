...
export const zh: Record<TKey, string> = {
  ...
  "models.change": "更改",
  "models.newSessionsOnly": "仅新会话",
  "models.modeLabel_v1": "经典 v1",
  "models.modeLabel_default": "遵循 Codex 默认值",
  ...
  "models.catalogBehaviorHint": "隐藏的模型会从目录和选择器中移除，但仍可通过精确 ID 调用。更改会在下一次 Codex 对话轮次生效，无需重启。",
  "models.custom": "自定义…",
  ...
  "sub.saved": "已保存 {n} 个快捷选择。启动新的 Codex 会话或运行 {cmd} 以刷新现有会话。",
  "sub.savedExcluded": "已保存 {n} 个快捷选择，但其中 {missing} 个当前未向 V2 工作器展示。",
  "sub.savedRefreshFailed": "已将 {n} 个快捷选择保存到磁盘，但运行中的目录未能正常刷新。在依赖新顺序之前请运行 {cmd}。",
  "sub.saveFailed": "保存失败",
  ...
  "sub.policy.messageDeliveryHint_plaintext": "实验功能。启用跨提供方 V2；该父级发送的所有 V2 工作者消息均为明文。更改后请启动新会话。",
  ...
  "sub.policy.concurrencyHint": "V2 计算包含根代理的总线程数；V1 计算子线程数。留空恢复 Codex 默认值，仅适用于新会话。",
  ...
  "sub.policy.saved": "运行策略已保存。协议和线程上限适用于新会话；V2 传递会影响后续请求，因此请启动新会话；指导和回退适用于之后生成的子任务。",
  ...
  "sub.policy.compatibilityV2": "外部提供方无法读取原生加密的 V2 任务（#92）。自动指导会过滤已知不兼容的工作者，显式覆盖会安全失败。跨提供方 V2 可选择明文兼容模式，或使用已验证的经典 v1 路径。",
  "sub.policy.compatibilityV2Plaintext": "已启用实验性的跨提供方 V2。CodexCommander 会转换原生协作协议，因此该父级的所有 V2 工作者消息（包括发给原生工作者的消息）均为明文。保存后请启动新会话；未知协议仍会安全失败。",
  ...
  "sub.catalog.nextSession": "下个会话生效",
  "sub.catalog.unknown": "目录状态未知",
  "sub.catalog.staleNotice": "保存的目录与 {n} 个正在运行的 Codex 会话不同。准备好重启这些会话并应用此名单时，请运行 {cmd}。",
  "sub.catalog.notRunningNotice": "没有正在运行的 Codex 会话使用此目录。保存的更改将在下个会话启动时生效。",
  "sub.roster.excludedNotice": "已保存的阵容模型当前未向 V2 spawn_agent 展示（{n} 个）：{models}。目录文件已加载，但该阵容尚未完全生效。",
