...
export const ru: Record<TKey, string> = {
  ...
  "sub.policy.saved": "Политика запуска сохранена. Протокол и лимит потоков применятся к новым сессиям, подсказки и fallback — к будущим дочерним задачам.",
  ...
  "sub.policy.compatibilityV2": "Совместимость оценивается для каждой задачи и родительского маршрута: нативный родитель ChatGPT может отправить зашифрованную V2-задачу, которую внешние провайдеры не прочитают (#92). При известной возможности маршрута автоматическая подсказка фильтрует кандидатов; явные overrides завершаются безопасной ошибкой. Для предсказуемого делегирования между провайдерами используйте Classic v1.",
