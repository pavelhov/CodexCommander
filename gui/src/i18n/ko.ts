...
export const ko: Record<TKey, string> = {
  ...
  "models.change": "변경",
  "models.newSessionsOnly": "새 세션에만 적용",
  "models.modeLabel_v1": "클래식 v1",
  "models.modeLabel_default": "Codex 기본값 따르기",
  ...
  "models.catalogBehaviorHint": "숨긴 모델은 카탈로그와 피커에서 제외되지만 정확한 ID로는 계속 호출할 수 있습니다. 변경 사항은 다음 Codex 턴부터 적용되며 재시작할 필요가 없습니다.",
  "models.custom": "직접 입력…",
  ...
  "sub.saved": "빠른 선택 {n}개를 저장했습니다. 새 Codex 세션을 시작하거나 {cmd} 를 실행해 기존 세션을 새로고침하세요.",
  "sub.savedExcluded": "빠른 선택 {n}개를 저장했지만 {missing}개는 현재 V2 워커에 표시되지 않습니다.",
  "sub.savedRefreshFailed": "빠른 선택 {n}개를 디스크에 저장했지만 실행 중인 카탈로그가 깔끔하게 새로고침되지 않았습니다. 새 순서에 의존하기 전에 {cmd} 를 실행하세요.",
  ...
  "sub.policy.messageDeliveryHint_plaintext": "실험적 기능입니다. 다중 프로바이더 V2를 활성화하며 이 부모의 모든 V2 워커 메시지를 평문으로 전송합니다. 변경 후 새 세션을 시작하세요.",
  ...
  "sub.policy.concurrencyHint": "V2는 루트를 포함한 전체 스레드 수, V1은 하위 스레드 수를 셉니다. 비워 두면 Codex 기본값으로 돌아가며 새 세션에 적용됩니다.",
  ...
  "sub.policy.saved": "실행 정책이 저장되었습니다. 프로토콜과 스레드 한도는 새 세션에 적용됩니다. V2 전달은 이후 요청부터 바뀌므로 새 세션을 시작하세요. 안내와 폴백은 이후 하위 작업에 적용됩니다.",
  ...
  "sub.policy.compatibilityV2": "외부 프로바이더는 네이티브 암호화 V2 작업을 읽을 수 없습니다 (#92). 자동 안내는 알려진 비호환 워커를 제외하고 명시적 오버라이드는 안전하게 실패합니다. 다중 프로바이더 V2에는 평문 호환 모드를, 검증된 경로에는 클래식 v1을 선택하세요.",
  "sub.policy.compatibilityV2Plaintext": "실험적 다중 프로바이더 V2가 활성화되었습니다. CodexCommander가 네이티브 협업 와이어를 변환하므로 네이티브 워커를 포함해 이 부모의 모든 V2 워커 메시지가 평문이 됩니다. 저장 후 새 세션을 시작하세요. 알 수 없는 스키마는 계속 안전하게 실패합니다.",
  ...
  "sub.catalog.nextSession": "다음 세션부터 적용",
  "sub.catalog.unknown": "카탈로그 상태 알 수 없음",
  "sub.catalog.staleNotice": "저장된 카탈로그가 실행 중인 Codex 세션 {n}개와 다릅니다. 세션을 재시작하고 이 로스터를 적용할 준비가 되면 {cmd} 를 실행하세요.",
  "sub.catalog.notRunningNotice": "이 카탈로그를 사용하는 실행 중인 Codex 세션이 없습니다. 저장된 변경 사항은 다음 세션 시작 시 적용됩니다.",
  "sub.roster.excludedNotice": "저장된 로스터 모델이 현재 V2 spawn_agent에 표시되지 않습니다({n}개): {models}. 카탈로그 파일은 로드됐지만 이 로스터는 완전히 적용되지 않았습니다.",
