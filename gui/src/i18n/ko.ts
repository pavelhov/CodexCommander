...
export const ko: Record<TKey, string> = {
  ...
  "sub.policy.saved": "실행 정책이 저장되었습니다. 프로토콜과 스레드 한도는 새 세션에, 안내와 폴백은 이후 하위 작업에 적용됩니다.",
  ...
  "sub.policy.compatibilityV2": "호환성은 작업과 부모 라우트별로 평가됩니다. 네이티브 ChatGPT 부모는 외부 프로바이더가 읽을 수 없는 암호화된 V2 워커 작업을 보낼 수 있습니다 (#92). 라우트 기능을 알 수 있으면 자동 안내가 후보를 걸러내고, 명시적 오버라이드는 안전하게 실패합니다. 예측 가능한 프로바이더 간 위임에는 클래식 v1을 사용하세요.",
