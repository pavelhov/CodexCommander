...
export const ja: Record<TKey, string> = {
  ...
  "models.change": "変更",
  "models.newSessionsOnly": "新しいセッションのみ",
  "models.modeLabel_v1": "クラシック v1",
  "models.modeLabel_default": "Codex の既定値に従う",
  ...
  "models.catalogBehaviorHint": "非表示モデルはカタログとピッカーから外れますが、正確な ID では引き続き呼び出せます。変更は次の Codex ターンで反映され、再起動は不要です。",
  "models.custom": "カスタム…",
  ...
  "sub.saved": "{n} 件のクイックピックを保存しました。新しい Codex セッションを開始するか {cmd} を実行して、既存セッションを更新してください。",
  "sub.savedExcluded": "{n} 件のクイックピックを保存しましたが、{missing} 件は現在 V2 ワーカーに表示されていません。",
  "sub.savedRefreshFailed": "{n} 件のクイックピックをディスクに保存しましたが、実行中のカタログが正常に更新されませんでした。新しい順序に依存する前に {cmd} を実行してください。",
  ...
  "sub.policy.messageDeliveryHint_plaintext": "実験的機能。複数プロバイダーの V2 を有効にし、この親からの全 V2 ワーカーメッセージを平文にします。変更後は新しいセッションを開始してください。",
  ...
  "sub.policy.concurrencyHint": "V2 はルートを含む総スレッド数、V1 は子スレッド数です。空欄で Codex の既定値に戻ります。新しいセッションに適用されます。",
  ...
  "sub.policy.saved": "実行ポリシーを保存しました。プロトコルとスレッド上限は新しいセッションに適用されます。V2 配信は次のリクエストから変わるため、新しいセッションを開始してください。ガイダンスとフォールバックは今後の子タスクに適用されます。",
  ...
  "sub.policy.compatibilityV2": "外部プロバイダーはネイティブの暗号化 V2 タスクを読めません (#92)。自動ガイダンスは既知の非互換ワーカーを除外し、明示的なオーバーライドは安全に失敗します。複数プロバイダーの V2 には平文互換モード、確立済みの経路には Classic v1 を選んでください。",
  "sub.policy.compatibilityV2Plaintext": "実験的な複数プロバイダー V2 が有効です。CodexCommander がネイティブ連携ワイヤーを変換するため、この親からのすべての V2 ワーカーメッセージ（ネイティブ宛ても含む）は平文になります。保存後に新規セッションを開始してください。未知のスキーマは引き続き安全に失敗します。",
  ...
  "sub.catalog.nextSession": "次のセッションから適用",
  "sub.catalog.unknown": "カタログの状態は不明です",
  "sub.catalog.staleNotice": "保存されたカタログが実行中の {n} 件の Codex セッションと異なります。それらを再起動してこのロースターを適用する準備ができたら {cmd} を実行してください。",
  "sub.catalog.notRunningNotice": "このカタログを使用している実行中の Codex セッションはありません。保存した変更は次のセッション開始時に適用されます。",
  "sub.roster.excludedNotice": "保存済みロースターのモデルが現在 V2 spawn_agent に表示されていません（{n} 件）：{models}。カタログファイルは読み込まれていますが、このロースターは完全には有効ではありません。",
