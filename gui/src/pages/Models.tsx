        ...
        <div className="models-advanced-body" aria-busy={!shadowCall || undefined}>
          <div className="models-shadow-row row text-control">
            ...
            <div className="models-shadow-model-slot">
              <Select value={shadowCall?.model ?? ""} options={[{ value: "", label: "\u2014" }, ...shadowModelOptions]} onChange={value => { setShadowCall(current => current ? { ...current, model: value } : current); void saveShadowCall({ model: value }); }} disabled={!shadowCall || shadowCallSaving || !shadowCall.enabled} label={t("models.shadowCallIntercept")} />
            </div>
          </div>
        </div>
      )}
